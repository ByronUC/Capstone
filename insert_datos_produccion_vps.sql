-- ====================================================================
-- SCRIPT DE INSERCIÓN DE DATOS PARA PRODUCCIÓN VPS
-- Sistema de Reservas - Conectemos Chile
-- ====================================================================
-- Compatible con el esquema existente en la VPS
-- ====================================================================
...
-- Regiones de Chile (solo si no existen)
...
-- Ciudades principales (solo si no existen)
...
('DIPRECA', 11),
('Particular', 12)
ON CONFLICT (id_prevision) DO NOTHING;
...
-- Estados de cita (usando color_identificador en lugar de color_hex)
INSERT INTO estados_cita (nombre_estado, descripcion, color_identificador, orden, id_estado_cita) VALUES
...
ON CONFLICT (id_psicologo) DO NOTHING;
...
SELECT setval('regiones_id_region_seq', (SELECT MAX(id_region) FROM regiones), true);
SELECT setval('ciudades_id_ciudad_seq', (SELECT MAX(id_ciudad) FROM ciudades), true);
SELECT setval('especialidades_id_especialidad_seq', (SELECT MAX(id_especialidad) FROM especialidades), true);
SELECT setval('previsiones_id_prevision_seq', (SELECT MAX(id_prevision) FROM previsiones), true);
SELECT setval('servicios_id_servicio_seq', (SELECT MAX(id_servicio) FROM servicios), true);
SELECT setval('estados_cita_id_estado_cita_seq', (SELECT MAX(id_estado_cita) FROM estados_cita), true);
SELECT setval('salas_atencion_id_sala_seq', (SELECT MAX(id_sala) FROM salas_atencion), true);
...
SELECT setval('psicologos_id_psicologo_seq', (SELECT MAX(id_psicologo) FROM psicologos), true);
